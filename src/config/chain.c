 ...
 * $Id: 7d0b71d789c5be7ca3d0c381a2bef9fcc1809550 $
 *
...
//        char name[plen + strlen( optname )];
        char *name = (char *)malloc(plen + strlen( optname ));			// sunqueen modify
//        snprintf( name, sizeof (name), "%s%s", psz_prefix, optname );
        snprintf( name, plen + strlen( optname ), "%s%s", psz_prefix, optname );			// sunqueen modify
        if( var_Create( p_this, name,
                        config_GetType( p_this, name ) | VLC_VAR_DOINHERIT ) )
		{
			free(name);			// sunqueen add
            return /* VLC_xxx */;
    	}
		free(name);				// sunqueen add
    }
...
//        char name[plen + strlen( ppsz_options[i] )];
        char *name = (char *)malloc(plen + strlen( ppsz_options[i] ));			// sunqueen modify
        const char *psz_name = name;
//        snprintf( name, sizeof (name), "%s%s", psz_prefix,
        snprintf( name, plen + strlen( ppsz_options[i] ), "%s%s", psz_prefix,			// sunqueen modify
